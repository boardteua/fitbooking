...
class frontend
{
    ...
    public function enqueue_scripts()
    {
        ...
        wp_localize_script($this->fit_plugin, 'fit',
            array(
                ...
                'rooms' => $this->utils->get_rooms(0),
                'trainers' => $this->utils->get_trainers(0),
                'crns' => $this->utils->get_currency()
            )
        );
    }
    ...
    public function update_event_ajax()
    {
        ...
        $pool = implode(',', $_POST['places_pool']);

        return $this->update_db_event(array('id' => $id, 'places_pool' => $pool));

    }

    protected function update_db_event($input)
    {
        global $wpdb;
